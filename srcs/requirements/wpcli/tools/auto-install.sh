...
if ! $(wp core is-installed --path="/var/www"); then
    ...
    su -c 'wp config create --skip-check --path="/var/www" --dbname=$WP_DB_NAME --dbuser=$MYSQL_USER --dbpass=$MYSQL_PASSWORD --dbhost=mariadb --dbcharset="utf8" --dbcollate="utf8_general_ci"' - root
    echo 'Config built'
